...
class DosenService {
  ...
  public static async checkExistingSetoran(nim: string, email_dosen_pa: string, nomorSurahInt: number) {
    const result = await prisma.$queryRaw`
      SELECT * FROM "setoran" 
      WHERE "nim" = ${nim} 
  ...
  public static async postSetoran(nim: string, email_dosen_pa: string, nomor_surah: number, tgl_setoran: string) {
